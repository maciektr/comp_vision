...
class Tracker(object):
    ...
    def _filter_old_tracks(self, timestamp: int) -> List[Track]:
        ...
        self._tracks = list(
            filter(
                lambda track: timestamp - track.last_timestamp <= self._config.max_age,
                self._tracks,
            )
        )
        return self._tracks
    ...
    def _assign_tracks(
        self, persons: List[Person], sim_matrix: List[List[float]], timestamp: int
    ) -> None:
        ...
        for detection_index in detection_indices:
            ...
            for track_index in unmatched_track_indices:
                ...
                if (
                    similarity >= self._config.min_similarity
    ...
    def _create_track(
        self, person: Person, timestamp: int, track_id: int = None
    ) -> Track:
