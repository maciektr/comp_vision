...
class BoundingBoxTrackerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.tracker_config = TrackerConfig(
            KeypointTrackerConfig(), _MAX_TRACKS, _MAX_AGE, _MIN_SIMILARITY
        )
    ...
    def test_iou(self):
        ...
        track = Track(Person([], Rectangle(Point(0, 1 / 3), Point(1, 1)), 1), 1000000)
    ...
    def test_iou_no_intersection(self):
        ...
        track = Track(Person([], Rectangle(Point(0.5, 0.5), Point(1, 1)), 1), 1000000)
        computed_iou = self.bbox_tracker._iou(person, track)
