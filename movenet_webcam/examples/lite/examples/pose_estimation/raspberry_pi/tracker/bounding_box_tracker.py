...
class BoundingBoxTracker(Tracker):
    ...
    def _iou(self, person: Person, track: Track) -> float:
        ...
        """
        x_min = max(
            person.bounding_box.start_point.x, track.person.bounding_box.start_point.x
        )
        y_min = max(
            person.bounding_box.start_point.y, track.person.bounding_box.start_point.y
        )
        x_max = min(
            person.bounding_box.end_point.x, track.person.bounding_box.end_point.x
        )
        y_max = min(
            person.bounding_box.end_point.y, track.person.bounding_box.end_point.y
        )
