...
class Posenet(object):
    ...
    def _process_output(
        self, heatmap_data: np.ndarray, offset_data: np.ndarray
    ) -> np.ndarray:
        ...
        for idx in range(joint_num):
            ...
            keypoints_with_scores[idx, 0] = (
                remap_pos[0] + offset_data[max_val_pos[0], max_val_pos[1], idx]
            ) / 257
            keypoints_with_scores[idx, 1] = (
                remap_pos[1]
                + offset_data[max_val_pos[0], max_val_pos[1], idx + joint_num]
            ) / 257
