...
class MovenetTest(unittest.TestCase):
    def setUp(self):
        ...
        self.keypoints_truth_1 = (
            pose_landmarks_truth.iloc[0].to_numpy().reshape((17, 2))
        )
        self.keypoints_truth_2 = (
            pose_landmarks_truth.iloc[1].to_numpy().reshape((17, 2))
        )

    def _detect_and_assert(
        self, detector: Movenet, image: np.ndarray, keypoints_truth: np.ndarray
    ) -> None:
        ...
        for idx in range(len(BodyPart)):
            ...
            logging.debug(
                "Detected %s close to expected result (%d)",
                BodyPart(idx).name,
                int(distance),
            )
