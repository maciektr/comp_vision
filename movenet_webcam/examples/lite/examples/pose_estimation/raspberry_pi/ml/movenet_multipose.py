...
class MoveNetMultiPose(object):
    ...
    def __init__(
        self, model_name: str, tracker_type: str = "bounding_box", input_size: int = 256
    ) -> None:
        ...
        if tracker_type == "keypoint":
            ...
            print(
                "ERROR: Tracker type {0} not supported. No tracker will be used.".format(
                    tracker_type
                )
            )
            self._tracker = None
    ...
    def detect(
        self, input_image: np.ndarray, detection_threshold: float = 0.11
    ) -> List[Person]:
        ...
        if is_dynamic_shape_model:
            ...
            resized_image = cv2.resize(
                input_image, (self._input_width, self._input_height)
            )
            input_tensor = np.expand_dims(resized_image, axis=0)
        ...
        self._interpreter.set_tensor(
            self._input_details[0]["index"], input_tensor.astype(self._input_type)
        )
        ...
        return self._postprocess(
            model_output, image_height, image_width, detection_threshold
        )
    ...
    def _postprocess(
        ...
        for idx in range(num_instances):
            ...
            for i in range(scores.shape[0]):
                keypoints.append(
                    KeyPoint(
                        BodyPart(i),
                        Point(
                            int(kpts_x[i] * image_width), int(kpts_y[i] * image_height)
                        ),
                        scores[i],
                    )
                )
            ...
            rect = [
                keypoints_with_scores[0, idx, 51],
                keypoints_with_scores[0, idx, 52],
                keypoints_with_scores[0, idx, 53],
                keypoints_with_scores[0, idx, 54],
            ]
